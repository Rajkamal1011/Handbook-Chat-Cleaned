    ...
    messages = [
        {
            ...
            "content": """You are a helpful chat assistant for students. Provide answers based solely on the provided handbook information."""
                          #If the answer to the query does not lie within the contextual information provided, respond with:
                          #\"As far as my knowledge, the handbook doesn't contain an answer to this query.\""""
        },
        {
            "role": "user",
...
def retrieve_relevant_resources(query: str, embeddings: torch.tensor, model: SentenceTransformer = embedding_model,
    ...
    #cos_sim_scores = util.cos_sim(query_embedding, embeddings)[0] 
...
textual_output_df.to_excel("textual_gold_testset_openai_no_as_far_as.xlsx", index=False)
...
tabular_output_df.to_excel("tabular_testset_openai_no_as_far_as.xlsx", index=False)

print("Responses generated and saved to 'textual_gold_testset_openai_no_as_far_as.xlsx' and 'tabular_testset_openai_no_as_far_as.xlsx'")
'''
...
vague_output_df.to_excel("vague_testset_openai_no_as_far_as.xlsx", index=False)
'''
...
# output_text, context_items = ask("What is the procedure to convert an Audited course to a Credited Course?")
output_text, context_items = ask("What are the course credits requirements for a MTech student in Chemical Sciences?")
print(output_text)
for context_item in context_items:
    print(f"Page = {context_item['page_number']}, Sim Score = {context_item['sim_score']}, Sentence Chunk = {context_item['sentence_chunk']}")
